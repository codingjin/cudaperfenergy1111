...
__global__ void matmul(const float *A, const float *B, float *C, const int M, const int N, const int K)
{
    ...
    // Prefetch first tile
    as[write_idx][threadIdx.y][threadIdx.x] = (row < M && threadIdx.x < K) ? A[row * K + threadIdx.x] : 0.0f;
    bs[write_idx][threadIdx.y][threadIdx.x] = (threadIdx.y < K && col < N) ? B[threadIdx.y * N + col] : 0.0f;
    __syncthreads();
    ...
    for (int kt = 0; kt < K; kt += BLOCKSIZE) {
        ...
        // Load next tile into write buffer (if not the last iteration)
        if (kt + BLOCKSIZE < K) {
            as[write_idx][threadIdx.y][threadIdx.x] = (row < M && (kt + BLOCKSIZE + threadIdx.x) < K) ?
                A[row * K + kt + BLOCKSIZE + threadIdx.x] : 0.0f;
            bs[write_idx][threadIdx.y][threadIdx.x] = ((kt + BLOCKSIZE + threadIdx.y) < K && col < N) ?
                B[(kt + BLOCKSIZE + threadIdx.y) * N + col] : 0.0f;
        }

        // Compute using read buffer
        for (int k = 0; k < BLOCKSIZE; ++k) {
            result += as[read_idx][threadIdx.y][k] * bs[read_idx][k][threadIdx.x];
        }
...
int main(int argc, char **argv)
{
    ...
    printf("Matrix-multiplication (Double-Buffered Energy Measurement): A(%d x %d) * B(%d x %d) = C(%d x %d)\n", M, K, K, N, M, N);
