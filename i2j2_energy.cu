...
__global__ void matmul(const float *A, const float *B, float *C, const int M, const int N, const int K)
{
    ...
    __shared__ float bs0[BLOCKSIZE][BLOCKSIZE];
    __shared__ float bs1[BLOCKSIZE][BLOCKSIZE];
    ...
    for (int kt = 0; kt < K; kt += BLOCKSIZE) {
        ...
        bs0[threadIdx.y][threadIdx.x] = ((kt + threadIdx.y) < K && col < N) ? B[(kt + threadIdx.y) * N + col] : 0.0f;
        bs1[threadIdx.y][threadIdx.x] = ((kt + threadIdx.y) < K && col + BLOCKSIZE < N) ? B[(kt + threadIdx.y) * N + (col + BLOCKSIZE)] : 0.0f;
        __syncthreads();

        for (int k = 0; k < BLOCKSIZE; ++k) {
            result00 += as0[threadIdx.y][k] * bs0[k][threadIdx.x];
            result01 += as0[threadIdx.y][k] * bs1[k][threadIdx.x];
            result10 += as1[threadIdx.y][k] * bs0[k][threadIdx.x];
            result11 += as1[threadIdx.y][k] * bs1[k][threadIdx.x];
        }
        __syncthreads();
    }
...
int main(int argc, char **argv)
{
    ...
    printf("Matrix-multiplication (i2j2 Energy Measurement): A(%d x %d) * B(%d x %d) = C(%d x %d)\n", M, K, K, N, M, N);
    ...
    dim3 grid((M + BLOCKSIZE - 1) / BLOCKSIZE / 2, (N + BLOCKSIZE - 1) / BLOCKSIZE / 2);
